...
if( !class_exists( 'wpForo' ) ) {
	...
	final class wpForo{
		...
		private function use_mysql8() {
			...
			if( preg_match( '#([\d.]+)-MariaDB#iu', $wpdb->get_var( "SELECT VERSION()" ), $match ) ){
				...
				$use_mysql8 = version_compare( $wpdb->db_version(), '8.0.0', '>=' );
			}
		...
		public function init_current_object(){
			...
			if( wpfval($current_object, 'topic_slug') ){
				...
				if(!empty($topic)){
                    ...
                    $protect = apply_filters( 'wpforo_current_object_topic_protect', true, $topic );
	...
	WPF()->addons = array(
        ...
        'woomem' => array('version' => '1.0.0', 'requires' => '1.9.9.1', 'class' => 'wpForoWooMembers', 'title' => 'WooCommerce Memberships Integration', 'thumb' => WPFORO_URL . '/wpf-assets/addons/woomem/header.png', 'desc' => __('Powers your forum with access control to forums and topics. Lets you earn having a membership protected forum.', 'wpforo'), 'url' => 'https://gvectors.com/product/wpforo-woocommerce-memberships/'),
